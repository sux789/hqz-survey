...
3. 不懂结构先跑探测，不猜：
   - Excel 模板结构 → 用 openpyxl 打印 sheet 名/表头行/合并区/前 N 行非空单元格
   - GDB 结构 → `python tools/verify_gdb.py <.gdb目录路径>`（图层/字段别名/坐标转换探测）
...
2. 涉及导出逻辑：用一个真实项目导出 base+samples，人工抽查关键单元格（合格率数值、坐标全精度、A2 小班号、sheet 名）。
