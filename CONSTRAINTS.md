...
- F8 后台持续轨迹（2026-08-25 二版，反转 R13）：**自研原生插件 BgLocation**（android/.../BgLocationPlugin.java + BgLocationService.java），平台 LocationManager（GPS_PROVIDER+NETWORK_PROVIDER 各 1s）+ location 型前台服务——通用方案，无 Google Play Services 依赖。**R13 教训：@capacitor-community/background-geolocation 依赖 gms FusedLocationProviderClient，在无 GMS 华为/鸿蒙上静默零回调（服务跑、通知在、点不来），已卸载弃用**。JS 侧 `Capacitor.Plugins.BgLocation.startWatcher({title,message}, cb)`：RETURN_CALLBACK 方法经 nativeCallback **同步返回 callbackId 字符串而非 Promise（不可 .then/.catch，R14）**；回调两参 `(loc, err)`，loc 携带 `provider` 字段（'gps'=WGS-84 / 'network'=GCJ-02），`_gpsFix` 优先按 provider 纠偏、无 provider（WebView 点）回退 accuracy≥50m 启发式；插件缺失（浏览器/旧 APK）/启动异常回退 watchPosition 仅前台；停止 `stopWatcher()` 停前台服务，授权弹窗期间停止由原生 wantsStop 标志丢弃防泄漏；AndroidManifest 已声明 service(foregroundServiceType=location) + FOREGROUND_SERVICE(_LOCATION)/POST_NOTIFICATIONS；华为设备仍需手动设启动管理允许后台运行（厂商行为）；轨迹精度策略（2026-08-25 终版，R15 反转）：**纯 GPS 采集**——原生插件仅注册 GPS_PROVIDER，网络兜底彻底移除（用户决策：网络点纠偏后绝对精度仍 50~1000m，是锯齿/偏移首点之源，宁可 GPS 无 fix 期间轨迹空白也不记偏点；GPS 冷启动 30s~几分钟/深林遮挡期间 0 点为预期行为，时间戳可查），GPS minDistance=2m 抑制静止抖动毛团；原生插件模式跳过 WebView getCurrentPosition 立即取首点；acc 已随点存档可事后甄别；林下弱 GPS 多径（5~30m）为物理极限；浏览器 watchPosition 回退模式仍走 WebView 定位（无法控制 provider，accuracy≥50m 启发式纠偏仍生效）
