...
- C9 小班查数株数（基本信息 AN/BE/AR 列 + 前端 computed）= 调查总株数（样地模板 B34 同口径）= round(Σ种植÷个数÷150×单个网格面积×种植网格数量)；个数=手写 sm_total_count（>0生效）回退实际样地数；合格株树=round(查数株数×合格率÷100) 随之联动（2026-08-21 反转旧Σ种植口径，见 R8）
